!(function() {
  ...
  var formatArea = function(a) {
    ...
    if (a < 100) {
      return '平面总面积：' + a.toFixed(1) + unit;
    } else {
      return  '平面总面积：'  + Math.round(a) +  unit;
    }
  }
  ...
  L.Polyline.include({
    showMeasurements: function(options) {
      ...
      this._measurementOptions = L.extend({
        ...
        minPixelDistance:30, // 打开后在移动过程中就会出现测量的值
        showDistances: true,
        showArea: true,
        lang: {
    ...
    updateMeasurements: function() {
      ...
      if (this._measurementOptions.showDistances && latLngs.length > 1) {
        ...
        for (var i = 1, len = latLngs.length; (isPolygon && i <= len) || i < len; i++) {
          ...
          if (pixelDist >= options.minPixelDistance && this instanceof L.Polygon) {
